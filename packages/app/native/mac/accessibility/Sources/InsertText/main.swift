...
func main() {
  ...
  if forced != .keyboard {
    if let focused = focusedElement(ofPid: pid), insertViaAccessibility(text, into: focused) {
  ...
  if insertViaKeyboard(text, pid: pid) {
...
/// 键盘路径：按行拆分，行内按 20 个 UTF-16 单元分块投递，换行单独发 Return 键
///
...
func insertViaKeyboard(_ text: String, pid: pid_t) -> Bool {
  ...
  for (index, line) in lines.enumerated() {
    ...
    if index < lines.count - 1 {
      guard pressKey(returnKeyCode, source: source, pid: pid) else { return false }
    }
  }
  return true
}
...
func pressKey(_ keyCode: CGKeyCode, source: CGEventSource, pid: pid_t) -> Bool {
