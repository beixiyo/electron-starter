...
export type ServiceHandlers<C extends IpcContract> = {
  [K in keyof C['mainHandle']]: C['mainHandle'][K] extends (...args: infer A) => infer R
...
export type ServiceListeners<C extends IpcContract> = {
  [K in keyof C['mainOn']]: C['mainOn'][K] extends (...args: infer A) => any
    ? (event: Electron.IpcMainEvent, ...args: A) => void
    : never
}
...
export type ServiceImpl<C extends IpcContract>
  = ([keyof C['mainHandle']] extends [never]
    ? { mainHandle?: never }
    : { mainHandle: ServiceHandlers<C> })
  & ([keyof C['mainOn']] extends [never]
    ? { mainOn?: never }
 ...
 */
export type IpcClient<C extends IpcContract>
  = InvokeMethods<C> & EventSubscription<C> & SendMethod<C>
