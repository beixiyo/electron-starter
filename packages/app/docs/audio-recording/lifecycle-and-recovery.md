...
- `stop`：采集停止、处理队列排空、文件关闭、clean/raw promotion、混音和校验，然后发送匹配 handoff 的 stopped
...
算法层的失败不能让已经写好的正式录音变成 terminal error。`fail` 只适合离线实验诊断，生产边界固定使用 raw fallback
...
- stop 的算法工作必须是实时录音期间完成的 O(1) 收尾。整场重算会随录音长度增长，并可能超过主进程 handoff 预算
