...
export const LoadingIcon = memo<LoadingIconProps>((
  ...
  const resolved = isStr(size)
    ? sizeMap[size]
    : size
  const resolvedThickness = thickness ?? borderWidth ?? Math.max(2, Math.round(resolved / 10))

  /**
   * 默认渲染渐变环；旧版 borderWidth 单独传入时保留单色环，显式 gradient
   * 配置优先于旧版宽度参数。color 继续沿用旧语义，传入后固定使用单色环
   */
  const isGradient = !ring
    && color === undefined
    && gradient !== false
    && (gradient !== undefined || borderWidth === undefined)
  ...
  const {
    color: ringColor = color ?? 'rgb(var(--text) / 0.5)',
...
export type LoadingIconBaseProps = {
  ...
  /**
   * 旧版环宽参数，保留用于兼容已有调用方
   * @deprecated 请使用 thickness
   */
  borderWidth?: number
  /**
   * 旧版单色环颜色参数；传入时自动使用 ring 形态
   * @deprecated 请使用 ring
   */
  color?: string
  /**
